...
- **`milestones.md`** — the phase order. Phase 1 (settings/staff/allocation) is done — see "Current state" below. Phase 2 (students/fees) is next.
...
- `modules/schools/` — profile, sessions/terms (collapsible + lazy-load pattern, single "current term"), sections, classes (drag-reorder), subjects (per-subject max scores), grading scale, number sequences
...
- `db/migrations/` — 5 migration files, SQLite-and-MySQL-compatible syntax only
...
- `views/settings/` — SchoolProfile, AcademicCalendar, ClassesSections, Subjects, GradingScale, NumberSequences — all real, all API-wired
- `views/staff/` — StaffList, Allocation

**Not built yet:** everything from Phase 2 onward (students, fees, results, promotion, accounting, recruitment, dashboards module, developer console).
...
See `README.md` for the exact commands. Short version: `cd server && npm install && npx knex migrate:latest && npx knex seed:run && npm run dev`, then `cd client && npm install && npm run dev`. Seeded login: `admin@educore.dev` / `changeme123`.
